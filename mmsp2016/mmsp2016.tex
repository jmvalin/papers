...
approaches used in video coding. In this paper, we describe the technology
behind Daala and discuss where it fits in the newly created AV1 codec. We
also present results that show that Daala is approaching the performance
level or more mature, state-of-the art video codecs. 
\end{abstract}


\section{Introduction}

Daala~\cite{DaalaWebsite} is a royalty-free video codec designed to
avoid traditional
...
were not used in popular video coding standards. Although Daala is not yet
...
blocks, and neither partition imposes any constraints on the sizes of the other. We also
experimented with $4\times 4$ MC blocks, but they were rarely used in practice,
...
evaluated for use in AV1. Using the piecewise
...
\caption{PSNR}
\end{subfigure}\hspace{0.1\columnwidth}\begin{subfigure}[t]{0.45\textwidth}
...
wth the AV1 encoder\footnote{Git version 337b23a5 from April 7th, 2016 with deringing enabled and command-line options: --ivf --frame-parallel=0 --tile-columns=0 --auto-alt-ref=2 --cpu-used=0 --passes=2 --threads=1 --kf-min-dist=1000 --kf-max-dist=1000 --lag-in-frames=25 --end-usage=q --cq-level=}, the x264 H.264
...
to PVQ activity masking, but the behavior is present even when activity masking
is not used.
...
encouraging. Moreover, these results have been steadily improving over the past
two years, suggesting that Daala may be a viable approach to royalty-free codecs
in the longer term. Some of the techniques presented in this paper should make their
